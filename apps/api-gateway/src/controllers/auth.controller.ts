...
export class AuthController {
  ...
  @ApiOperation({
    summary: 'User Login',
    description: 'Authenticate user with username/email and password to get access tokens'
  })
  @ApiBody({
    type: LoginDto,
  ...
  async refreshToken(@Body() refreshTokenDto: RefreshTokenDto): Promise<ApiResponseDto<{ accessToken: string; refreshToken: string }>> {
  ...
  async validateToken(@Body() validateTokenDto: ValidateTokenDto): Promise<ApiResponseDto<TokenValidationResponseDto>> {
  ...
  async logout(@Headers('authorization') authHeader: string): Promise<ApiResponseDto<null>> {
  ...
  async getUserProfile(@Param() params: GetUserProfileDto): Promise<ApiResponseDto<UserProfileResponseDto>> {
    try {
  ...
  @ApiOperation({
    ...
    description: 'Update user profile information (first name, last name, email)'
  })
  @ApiBearerAuth()
  @ApiParam({
    name: 'userId',
